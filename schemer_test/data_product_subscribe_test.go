...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.Given(`^Publish an Event with payload '{"string":"'([^'"]*?)'","binary":"'([^'"]*?)'","int":"'([^'"]*?)'","uint":"'([^'"]*?)'","float":"'([^'"]*?)'","bool":"'([^'"]*?)'","time":"'time'","time.precision":"'time.precision'","array":"'array'","array.subtype":"'array.subtype'","map":"'map'","any":"'any'"}' into sdk$`, PublishProductEvent)
